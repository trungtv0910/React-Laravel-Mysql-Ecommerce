...
@section('content')
    <div class="content-wrapper">
        ...
        <form class="forms-sample" method="POST" action="{{route('role.update',['id'=>$dataRole->id])}}">
            @csrf
            <div class="row">
                        ...
                        <div class="card mt-3">
                            <div class="card-header">
                                <label for="_checkGroup{{$key}}">
                                    <input type="checkbox" id="_checkGroup{{$key}}"
                                           class="checkGroupPermission"> {{$permission->name}}
                                </label>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    @foreach($permission->handleGetPermissionChild  as $keyChild=> $perChild)
                                        <div class="col-md-3">
                                            <label for="_checkChild{{$key.$keyChild}}">
                                                <input type="checkbox"
                                                  id="_checkChild{{$key.$keyChild}}"
                                                  class="checkbox_children"
                                                  name="permission_id[]"
                                                  value="{{$perChild->id}}"
                                                       {{ $roleOfPermission->contains('id',$perChild->id)? 'checked':''  }}
                                                >
                                                {{$perChild->name}}
                                            </label>
                                        </div>
